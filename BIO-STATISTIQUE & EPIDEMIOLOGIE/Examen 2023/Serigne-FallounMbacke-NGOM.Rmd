...
```{r}
titanic23 = read.csv2("C:/Users/Serigne Fallou NGOM/Downloads/titanic_data23.csv",sep=",",dec=".", header=TRUE, check.names=FALSE)
head(titanic23)
```
...
# 4. Calculer les odd-ratio et interpréter les résultats
```{r}
exp(coef(myreg))
```
...
# 7. La moyenne des residus est de -0.5942 .
...
.
# 9. Construction arbre de decision :
```{r}
library(rpart)
library(rpart.plot)
tree.model <- rpart(survived ~ sex+fare, method="class", data=titanic23)
...
# 1. Importer la base:
```{r}
mtcars = readxl::read_xlsx("C:/Users/Serigne Fallou NGOM/Downloads/mtcars.xlsx")
head(mtcars)
```
